...
public class PickerViewAlone extends LinearLayout {
    ...
    private void setAloneData(ReadableArray array) {
        ArrayList<String> values = arrayToList(array, "content");
        ArrayList<String> codes = arrayToList(array, "code");
        Log.d(TAG, "setAloneData1: "+values);
        Log.d(TAG, "setAloneData2: "+codes);
//        switch (array.getType(0).name()) {
//            case "Map":
//                for (int i = 0; i < array.size(); i++) {
//                    ReadableMap map = array.getMap(i);
//                    Log.d(TAG, "setPickerData1: " + map);
//                    values.add(map.getString("content"));
//                    codes.add(map.getString("code"));
//                }
//                break;
//            default:
...
//        ArrayList<String> values = arrayToList(array);
        ...
        loopView.setListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(String item, int index, String Code) {
                if (onSelectedListener != null) {
                    ReturnData returnData1 = new ReturnData();
                    returnData1.setItem(item);
                    returnData1.setCode(Code);
        ...
        for (int i = 0; i < array.size(); i++) {
            switch (array.getType(i).name()) {
                case "Array":
                    ...
                    ArrayList<String> values = arrayToList(childArray,null);
                    final LoopView loopView = new LoopView(getContext());
    ...
    private ArrayList<String> arrayToList(ReadableArray array, String key) {
        ArrayList<String> values = new ArrayList<>();
        for (int i = 0; i < array.size(); i++) {
            String value = "";
            Log.d(TAG, "arrayToList: " + array.getType(i).name());
            switch (array.getType(i).name()) {
                case "Map":
                    ReadableMap map = array.getMap(i);
                    Log.d(TAG, "setPickerData1: " + map);
